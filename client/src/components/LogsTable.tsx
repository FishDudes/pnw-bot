...
import {
  ...
  Users, UserCheck, Clock, Eye, WifiOff, Wifi,
} from "lucide-react";
...
// Unified entry for the combined sorted view
type Entry =
...
export function LogsTable() {
  ...
  // Counts for the filter tabs (unfiltered by search)
  ...
  const trackingCount = (tracked ?? []).length;
  ...
  // Build a unified sorted list so most-recent entry always appears at the top
  const entries = useMemo((): Entry[] => {
    ...
    // Sort descending — most recent event first
    result.sort((a, b) => b.time - a.time);
    return result;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [logs, tracked, filter, search]);
          ...
          <tbody className="divide-y divide-white/5">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={6} className="px-5 py-10 text-center text-muted-foreground text-sm">
                  {filter === "tracking"
                    ? "No nations currently being tracked. Enable Timed mode to start tracking."
              ...
              entries.map((entry) => {
                if (entry.kind === "tracked") {
                  ...
                  return (
                    <tr key={`tracked-${t.id}`} className="hover:bg-white/5 transition-colors" data-testid={`row-tracked-${t.id}`}>
                      ...
                      <td className="px-3 sm:px-5 py-3 font-medium text-foreground max-w-[140px] sm:max-w-none">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                          <span className="truncate">{t.nationName}</span>
                          {isOffline ? (
                ...
                return (
                  <tr key={`log-${log.id}`} className="hover:bg-white/5 transition-colors" data-testid={`row-log-${log.id}`}>
                    ...
                    <td className="px-3 sm:px-5 py-3 font-medium text-foreground max-w-[140px] sm:max-w-none">
                      <span className="truncate block">{log.nationName}</span>
                    </td>
